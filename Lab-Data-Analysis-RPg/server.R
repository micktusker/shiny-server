...
shinyServer(function(input, output) {
  ...
  output$dynamicFilters <- renderUI({
    ...
    experiment.filtered.df <- subset(experiment.assay.datatype.dataframe, experiment.name = experiment.name)
    datatype.names <- sort(unique(experiment.filtered.df$datatype_name))
    tagList(
      selectInput('datatype_name', label = "Select data type", choices = datatype.names, multiple = FALSE),
  ...
  datatype.name <- reactive(input$datatype_name)
