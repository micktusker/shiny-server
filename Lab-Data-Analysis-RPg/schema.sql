...
CREATE UNLOGGED TABLE transit_tmp(
    data_row TEXT);
COMMENT ON TABLE transit_tmp IS 'an unlogged table for storing temporarily while it is processed. Data is expected to be tab-delimited.';
CREATE TABLE stored_data.loaded_files_metadata(
    source_excel_file_id TEXT PRIMARY KEY,
...
COMMENT ON COLUMN stored_data.loaded_files_metadata.source_excel_file_id IS 'Generated from the MD5 function of the value stored in column source_excel_file_path. Used as a foreign key in table stored_data.pan_tcell_facs_data.';
...
CREATE TABLE stored_data.pan_tcell_facs_data(
  ...
  source_excel_file_id TEXT NOT NULL REFERENCES stored_data.loaded_files_metadata(source_excel_file_id));
...
COMMENT ON COLUMN stored_data.pan_tcell_facs_data.source_excel_file_id IS 'Table foreign key. Can only load rows into this table if this identifier (MD5 of source Excel file path) exists in the referenced table';
...
CREATE OR REPLACE FUNCTION add_row_to_pan_tcell_facs_data(p_source_excel_file_id TEXT)
RETURNS INTEGER
AS
$$
DECLARE
...
BEGIN
  INSERT INTO stored_data.pan_tcell_facs_data(
      ...
      cd4_cell_number, cd8_cell_number, sample_identifier, source_excel_file_id)
  SELECT
    ...
    p_source_excel_file_id source_excel_file_id
  FROM
    (SELECT
      data_row
    FROM
...
$$
LANGUAGE plpgsql;
COMMENT ON FUNCTION add_row_to_pan_tcell_facs_data(TEXT) IS $qq$Purpose: Insert all the data rows loaded into "transit_tmp" from the VBA code output file into table "pan_tcell_facs_data". Example: SELECT add_row_to_pan_tcell_facs_data('7f07b8c8f7179e40aae58fc6b3713445'); Returns the number of rows inserted.$qq$
